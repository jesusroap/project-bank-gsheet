...
async function getCSVFByDocId(id, rango) {
    let url = `https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${rango}?key=${claveAPI}`;
...
function contenedorCarga() {
    jQuery("#contenedor_carga").fadeOut(2000);
    jQuery('body').removeClass('over-hidden');
};

getCSVFByDocId('1HsF7qvYq30TyWISZYdtx5Ka-L8XqRjcWddhBIHBTOOI', 'PaginaWeb!A:Z').then(procesaDatosAJSON).then(optionsDataTable).then(contenedorCarga);
